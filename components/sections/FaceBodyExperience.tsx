...
const faceZones: Zone[] = [
  ...
  { id: 'under-eye', label: 'underEye', path: 'M431.911 296.485C418.364 305.587 411.304 309.426 385.214 310.604C359.124 311.783 342.248 305.636 337.432 300.754C307.753 270.675 358.039 287.813 384.129 286.635C410.219 285.457 464.765 274.411 431.911 296.485Z', products: [products.proAgeFine, products.ey] },
  ...
  { id: 'side-eye', label: 'sideEye', path: 'M477.573 266.159C488.811 253.807 494.582 240.763 490.462 237.024C486.342 233.286 473.892 240.268 462.653 252.62C451.414 264.972 445.644 278.016 449.764 281.755C453.883 285.493 466.334 278.511 477.573 266.159Z', products: [products.proAgeFine] },
  ...
  { id: 'double-chin', label: 'doubleChin', path: 'M472 483.5C446 501 446.5 498.5 417 512.5C336 542 315.5 534.5 297 540.5C302 548.2 353.005 545.771 392 533.5C402.686 530.137 444.5 512.5 456.5 503.5C472 489.5 476 488 472 483.5Z', products: [products.faceSculpt, products.lipo] },
  { id: 'side-mouth', label: 'sideMouth', path: 'M350.5 486C358.508 486 365 479.508 365 471.5C365 463.492 358.508 457 350.5 457C342.492 457 336 463.492 336 471.5C336 479.508 342.492 486 350.5 486Z', products: [products.proAgeFine] },
  { id: 'smile-lines', label: 'smileLines', path: 'M365.535 430.367C363.6 455.225 355.947 422.395 338.457 416.061C323.587 410.676 298.422 383.312 325.238 388.462C343.642 391.997 350.221 400.573 352.316 402.768C363.395 414.386 365.959 424.914 365.535 430.367Z', products: [products.proAgeFine] },
  ...
  return (
    <section className="face-body-figure-section section-lg">
      <div className="face-body-figure-panel">
        <h2>{t(area === 'face' ? 'face' : 'body')}</h2>
        <div className="face-body-figure">
          <svg
            viewBox={area === 'face' ? '0 0 679 681' : '0 0 681 681'}
            role="img"
            aria-label={t(area === 'face' ? 'aria.selectFaceZone' : 'aria.selectBodyZone')}
          >
            <defs>
          ...
          {zones.map((zone) => (
            <path
              ...
              aria-label={t(`zones.${zone.label}`)}
            >
              <title>{t(`zones.${zone.label}`)}</title>
            </path>
          ))}
          </svg>
        </div>
...
function ProductPanel({ area, zones, activeZoneId, t }: { area: Area; zones: Zone[]; activeZoneId: string | null; t: any }) {
  ...
  return (
    <section ref={panelRef} className="face-body-products-section section-lg">
      <div className="face-body-products-panel">
        ...
        <div className="face-body-filter-row" role="group" aria-label={t('aria.productFilters')}>
          {filters.map((item) => (
            <button
              key={item}
              type="button"
            ...
            >
              {item === filler ? t('filters.fillers') : t('filters.mesotherapy')}
            </button>
          ))}
        </div>
        ...
        <div className="face-body-product-grid" aria-live="polite">
          {visibleProducts.map((product) => (
            <Link href={`/product/${product.slug}` as any} className="face-body-product-card" key={`${activeZone.id}-${product.slug}`}>
              <img src={product.image} alt={product.title} loading="lazy" />
              <span className="face-body-product-title">{product.title}</span>
              <span>{product.type === filler ? t('filters.fillers') : t('filters.sterileVials')}</span>
            </Link>
          ))}
        </div>
      </div>
...
export default function FaceBodyExperience() {
  const t = useTranslations('faceBody');
  ...
  return (
    <>
      <AreaFigure area="face" zones={faceZones} activeZoneId={activeFaceZone} onSelect={setActiveFaceZone} t={t} />
      <ProductPanel area="face" zones={faceZones} activeZoneId={activeFaceZone} t={t} />
      <AreaFigure area="body" zones={bodyZones} activeZoneId={activeBodyZone} onSelect={setActiveBodyZone} t={t} />
      <ProductPanel area="body" zones={bodyZones} activeZoneId={activeBodyZone} t={t} />
    </>
  );
}
